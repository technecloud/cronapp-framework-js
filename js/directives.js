    ...
    return {
      ...
      generateModalSaveOrCancelButtonCall: function(buttonType, functionToCall, datasourceName, modalId, scope) {
        ...
        var compileTemplateAngular = function(buttonType, functionToCall, datasourceName, modalId) {
          var template;
          if (buttonType == 'save')
            template = '<button id="#BUTTONID#" class="btn btn-primary ng-binding grid-save-button-modal btn-fab k-button" data-component="crn-button" ng-click="#FUNCTIONCALL#" onclick="(!#DATASOURCENAME#.missingRequiredField()?$(\'##MODALID#\').modal(\'hide\'):void(0))">{{"Save" | translate}}</button>';
          else
            template = '<button id="#BUTTONID#" type="button" class="btn btn-default ng-binding btn-fab k-button" data-component="crn-button" data-dismiss="modal">{{"Home.view.Close" | translate}}</button>'
          template = template
          .split('#BUTTONID#').join(buttonId)
